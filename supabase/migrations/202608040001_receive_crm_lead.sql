-- Etapa 4A.2 — receptor genérico e transacional de leads por webhook.
-- O endpoint público é a Edge Function receive-crm-lead. Esta migration
-- mantém toda a criação do contato, lead, campos personalizados e auditoria
-- dentro de uma única transação no banco.

create or replace function private.crm_webhook_has_value(
  p_values jsonb,
  p_key text
)
returns boolean
...
as $function$
  select case
    when jsonb_typeof(coalesce(p_values, '{}'::jsonb)) <> 'object' then false
    when not (coalesce(p_values, '{}'::jsonb) ? p_key) then false
    when p_values -> p_key is null then false
    when jsonb_typeof(p_values -> p_key) = 'null' then false
    when jsonb_typeof(p_values -> p_key) = 'string'
      then nullif(pg_catalog.btrim(p_values ->> p_key), '') is not null
    when jsonb_typeof(p_values -> p_key) = 'array'
      then jsonb_array_length(p_values -> p_key) > 0
    else true
  end;
$function$;

create or replace function private.crm_webhook_text_value(
  p_values jsonb,
  p_key text
)
returns text
...
as $function$
  select case
    when private.crm_webhook_has_value(p_values, p_key)
      then nullif(pg_catalog.btrim(p_values ->> p_key), '')
    else null
  end;
$function$;

create or replace function private.crm_webhook_decimal(
  p_value text
)
returns numeric
language plpgsql
...
begin
  ...
  if pg_catalog.strpos(v_value, ',') > 0
     and pg_catalog.strpos(v_value, '.') > 0 then
    v_value := pg_catalog.replace(v_value, '.', '');
    v_value := pg_catalog.replace(v_value, ',', '.');
  elsif pg_catalog.strpos(v_value, ',') > 0 then
    v_value := pg_catalog.replace(v_value, ',', '.');
  end if;
...
create or replace function public.record_crm_webhook_failure(
  p_integration_id uuid,
  p_request_id text,
  p_payload jsonb,
...
declare
  ...
  v_request_id text := nullif(pg_catalog.btrim(coalesce(p_request_id, '')), '');
  v_error text := left(
    coalesce(nullif(pg_catalog.btrim(p_error_message), ''), 'Falha ao receber lead.'),
    500
  );
...
begin
  ...
  if v_request_id is null then
    v_request_id := 'failed_' || gen_random_uuid()::text;
  end if;
  ...
  return jsonb_build_object(
    'recorded', true,
    'new_event', v_inserted > 0
  );
end;
$function$;

revoke all on function private.crm_webhook_has_value(jsonb, text) from public;
revoke all on function private.crm_webhook_text_value(jsonb, text) from public;
revoke all on function private.crm_webhook_decimal(text) from public;
revoke all on function public.ingest_crm_webhook_lead(uuid, text, jsonb, jsonb) from public;
revoke all on function public.record_crm_webhook_failure(uuid, text, jsonb, text) from public;

grant execute on function public.ingest_crm_webhook_lead(uuid, text, jsonb, jsonb) to service_role;
grant execute on function public.record_crm_webhook_failure(uuid, text, jsonb, text) to service_role;
